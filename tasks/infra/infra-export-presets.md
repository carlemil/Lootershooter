# HIGH — Export presets: client-windows and server-linux
...
## Issue
The server ships as a Docker image built by exporting a dedicated-server binary, and the Dockerfile task hard-codes the preset name `server-linux`. Neither preset exists yet, so `godot --headless --export-release "server-linux"` fails and the container cannot be built. The client also needs a Windows preset for local playtesting.

## Fix
- Create `export_presets.cfg` with exactly two presets, in this order:
...
- Create `tools/export.ps1`:
  - Params `-Preset <client-windows|server-linux>` (default both) and `-Debug` switch.
...
- `grep -c 'name="' export_presets.cfg` returns 2 and the names are exactly `client-windows` and `server-linux`.
